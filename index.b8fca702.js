...
class Game {
   ...
   */ constructor(initialState){
        this.score = 0;
        this.status = "idle";
        this.initialState = deepCopyArray(initialState || [
            [
                0,
                0,
                0,
    ...
    moveLeft() {
        ...
        this.move((row)=>row, (board, newRow, rowIndex)=>{
            for(let col = 0; col < 4; col++)board[rowIndex][col] = newRow[col];
        });
    }
    moveRight() {
        if (this.status !== "playing") return;
        this.move((row)=>row.slice().reverse(), (board, newRow, rowIndex)=>{
            for(let col = 0; col < 4; col++)board[rowIndex][col] = newRow[3 - col];
        });
    }
    moveUp() {
        if (this.status !== "playing") return;
        this.move((row, index)=>this.board.map((rowItem)=>rowItem[index]), (board, newRow, colIndex)=>{
            for(let row = 0; row < 4; row++)board[row][colIndex] = newRow[row];
        });
    }
    moveDown() {
        if (this.status !== "playing") return;
        this.move((row, index)=>this.board.map((rowItem)=>rowItem[index]).reverse(), (board, newRow, colIndex)=>{
            newRow.reverse();
            for(let row = 0; row < 4; row++)board[row][colIndex] = newRow[row];
        });
    }
    ...
    combine(inputRow) {
        ...
        let scoreDelta = 0;
        const newRow = [];
        for(let i = 0; i < row.length; i++)if (row[i] === row[i + 1]) {
            newRow.push(row[i] * 2);
            scoreDelta += row[i] * 2;
            row[i + 1] = 0;
            i++;
